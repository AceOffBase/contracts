/**
 *Submitted for verification at basescan.org on 2023-09-10
*/
...
// File: contracts/farm/IRewardToken.sol
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accPerShare;      // Accumulated rewards per share, times 1e12. See below.
    }
    ...
    constructor(IRewardToken _reward, address _treasury, uint256 _rewardPerSec) {
        token = _reward;
        treasury = _treasury;
    ...
    modifier onlyAdmin() {
        require(msg.sender == owner() || msg.sender == manager, "admin: wut?");
        _;
    }
    ...
    function setStartTime(uint256 _startTime) external onlyAdmin {
        ...
        require(block.timestamp < _startTime, "too late");
    ...
    function updateRewardPerSecond(uint256 _rewardPerSec) public onlyAdmin {
        require(_rewardPerSec < rewardPerSec, "no rug.");
        rewardPerSec = _rewardPerSec;
    }
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyAdmin nonDuplicated(_lpToken) {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function set(uint256 _pid, uint256 _allocPoint, bool _withUpdate) public onlyAdmin poolExists(_pid) {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 reward = multiplier.mul(rewardPerSec).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
        uint256 reward = multiplier.mul(rewardPerSec).mul(pool.allocPoint).div(totalAllocPoint);
        token.mint(treasury, reward.div(10));
        token.mint(address(token), reward);
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal {
        token.safeRewardTransfer(_to, _amount);
    }
}
